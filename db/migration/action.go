...
type Actionable interface {
	Action(context.Context, *db.Connection, *db.Tx) error
}

// Action is a function that can be run during a migration step.
type Action func(context.Context, *db.Connection, *db.Tx) error

// NoOp performs no action.
func NoOp(_ context.Context, _ *db.Connection, _ *db.Tx) error { return nil }

// Statements returns a body func that executes the statments serially.
func Statements(statements ...string) Action {
	return func(ctx context.Context, c *db.Connection, tx *db.Tx) (err error) {
