...
type GuardFunc func(context.Context, *db.Connection, *db.Tx, Action) error
...
func Guard(description string, predicate func(c *db.Connection, tx *db.Tx) (bool, error)) GuardFunc {
	return func(ctx context.Context, c *db.Connection, tx *db.Tx, step Action) error {
...
func Always() GuardFunc {
	return Guard("always run", func(_ *db.Connection, _ *db.Tx) (bool, error) { return true, nil })
}

// IfExists only runs the statement if the given item exists.
func IfExists(statement string) GuardFunc {
	return Guard("if exists run", func(c *db.Connection, tx *db.Tx) (bool, error) {
...
func IfNotExists(statement string) GuardFunc {
	return Guard("if not exists run", func(c *db.Connection, tx *db.Tx) (bool, error) {
...
func guardPredicate(description string, p predicate, arg1 string) GuardFunc {
	return Guard(description, func(c *db.Connection, tx *db.Tx) (bool, error) {
		return p(c, tx, arg1)
	})
}
...
type predicate func(*db.Connection, *db.Tx, string) (bool, error)

// predicate2 is a function that evaluates based on two string params.
type predicate2 func(*db.Connection, *db.Tx, string, string) (bool, error)

// predicate3 is a function that evaluates based on three string params.
type predicate3 func(*db.Connection, *db.Tx, string, string, string) (bool, error)
...
func Exists(c *db.Connection, tx *db.Tx, selectStatement string) (bool, error) {
...
func NotExists(c *db.Connection, tx *db.Tx, selectStatement string) (bool, error) {
	if !stringutil.HasPrefixCaseless(selectStatement, "select") {
		return false, fmt.Errorf("statement must be a `SELECT`")
	}
